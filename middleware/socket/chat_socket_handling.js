export default function (io, acceptChatConnection, chatRooms, maxCons) {
  let id;
  ...
  io.of("/chat").on("connection", function (socket) {
    ...
    if (!acceptChatConnection(socket)) {
      ...
      if (chatRooms[id]["numCons"] >= maxCons) {
        chatRooms[id]["status"] = 2;
      } else {
        chatRooms[id]["status"] = 1;
      }
    }
    ...
    socket.on("chat_join", function () {
      io.of("/chat")
        .to("chat" + socket.handshake.query["id"])
        .emit("is_online", "🔵 <i>" + username + " joined the chat.</i>");
    });
    ...
    socket.on("disconnect", function () {
      ...
      io.of("/chat")
        .to("chat" + socket.handshake.query["id"])
        .emit("is_online", "🔴 <i>" + username + " left the chat.</i>");
    ...
    socket.on("chat_message", function (message) {
      io.of("/chat")
        .to("chat" + socket.handshake.query["id"])
        .emit("chat_message", "<strong>" + username + "</strong>: " + message);
